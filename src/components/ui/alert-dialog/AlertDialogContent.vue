...
<template>
  <AlertDialogPortal>
    <AlertDialogOverlay data-slot="alert-dialog-overlay"
      class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 bg-black/10 duration-100 fixed inset-0 z-[var(--z-dialog-overlay)]"
    ...
    <AlertDialogContent data-slot="alert-dialog-content" :data-size="size" v-bind="{ ...$attrs, ...forwarded }" :class="cn(
      'pointer-events-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 transform-gpu group/alert-dialog-content fixed top-1/2 left-1/2 grid w-full max-w-[calc(100%-2rem)] max-h-[calc(100vh-3rem)] -translate-x-1/2 -translate-y-1/2 gap-6 overflow-y-auto rounded-xl border border-[var(--app-border-dark)] bg-popover p-6 text-[var(--app-font-body-size)] text-popover-foreground shadow-[var(--niri-shadow-dialog)] duration-[var(--app-motion-panel)] ease-[var(--app-motion-ease)] outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-lg',
      props.class,
    )
